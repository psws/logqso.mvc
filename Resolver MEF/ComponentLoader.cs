...
namespace Logqso.mvc.Resolver_MEF
{
    ...
    internal class RegisterComponent : IRegisterComponent
    {
        ...
        public void RegisterInstance<Tobj>(Tobj Tobject) where Tobj : class
        {
            this._container.RegisterInstance(Tobject);


        }

        public void RegisterTypeWithInjectionTypes<TFrom, TTo>(Object[] parms, bool withInterception = false) where TTo : TFrom
        {
            if (withInterception)
            {
                //register with interception
            ...
            else
            {
                ...
                //type an array list
                this._container.RegisterType<TFrom, TTo>(
                    new InjectionConstructor(parms));
            }
        }
